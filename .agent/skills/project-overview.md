...
- Unit tests = vitest 5, **all under `_tests_/` mirroring `src/`** (`_tests_/**/*.test.ts(x)`, import via `@/…`; 175 tests, coverage-gated at 90% via `pnpm test:coverage` (jsdom specs carry a `// @vitest-environment jsdom` pragma). E2E = Playwright (`@playwright/test`, exact) under `_tests_/e2e/app/**` matching `src/app/**` routes; config `playwright.config.ts` spawns `next start` on :3100 via `node_modules/.bin/` (PM-agnostic). CI `e2e` job runs it; sandbox cannot (browser CDN blocked) — use `playwright test --list` locally. Generated output has ZERO runtime npm deps.
...
- Pure logic lives in `scripts/buildEnv.ts` and is unit-tested under `_tests_/scripts/` (13 tests; suite is 175).
