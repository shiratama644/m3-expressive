...
export function M3Button({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`group m3e-press relative inline-flex h-10 shrink-0 select-none items-center justify-center gap-2 overflow-hidden rounded-(--m3e-shape-button) px-6 t-label-large active:scale-[0.95] active:rounded-(--m3e-shape-medium) disabled:pointer-events-none disabled:opacity-40 ${BUTTON_VARIANTS[variant]} ${className}`}
    >
      <span
        aria-hidden
...
export function M3IconButton({
  ...
  const sizes = { small: 'h-8 w-8 text-[18px]', medium: 'h-10 w-10 text-[22px]', large: 'h-12 w-12 text-[26px]' };
  const variants = {
    standard: 'text-on-surface-variant hover:bg-on-surface/8',
    filled: 'bg-primary text-on-primary hover:brightness-95',
    tonal: active ? 'bg-secondary text-on-secondary' : 'bg-secondary-container text-on-secondary-container hover:brightness-95',
  ...
  return (
    <button
      ...
      className={`group m3e-press relative inline-flex shrink-0 select-none items-center justify-center overflow-hidden rounded-(--m3e-shape-icon-button) active:scale-90 active:rounded-(--m3e-shape-medium) ${sizes[size]} ${variants[variant]}`}
    >
...
export function M3ButtonGroup() {
  ...
  return (
    <div className="m3e-press flex w-full overflow-hidden rounded-(--m3e-shape-button) border border-outline-variant">
      {items.map((item, i) => (
        <button
          key={item.label}
          type="button"
          onClick={() => setSelected(i)}
          className={`group m3e-press flex h-12 flex-1 items-center justify-center border-outline-variant active:scale-y-[0.92] t-label-large ${
            i > 0 ? 'border-l' : ''
...
export function M3SplitButton() {
  return (
    <span className="m3e-press inline-flex items-stretch overflow-hidden rounded-(--m3e-shape-button)">
      <button
        type="button"
        className="group m3e-press relative flex items-center gap-2 rounded-l-(--m3e-shape-button) bg-tertiary px-5 text-on-tertiary active:scale-y-[0.94] t-label-large"
      >
      ...
      <button
        ...
        className="group m3e-press relative flex w-12 items-center justify-center rounded-r-(--m3e-shape-button) bg-tertiary-container text-on-tertiary-container hover:brightness-95 active:scale-y-[0.94]"
      >
...
export function M3Fab({ size = 'large', extended = false, icon = 'edit', label }: { size?: 'small' | 'medium' | 'large'; extended?: boolean; icon?: string; label?: string }) {
  ...
  return (
    <button
      ...
      className={`group m3e-press relative inline-flex shrink-0 select-none items-center justify-center gap-2 overflow-hidden rounded-(--m3e-shape-fab) bg-primary-container text-on-primary-container shadow-sm transition-shadow hover:shadow-md active:scale-90 active:rounded-(--m3e-shape-extra-large) ${
...
export function M3SegmentedButtons({ options, value = 0 }: { options: string[]; value?: number }) {
  ...
  return (
    <div role="group" className="flex h-10 overflow-hidden rounded-(--m3e-shape-button) border border-outline">
      {options.map((label, i) => (
        <button
          key={label}
          type="button"
          ...
          onClick={() => setSelected(i)}
          className={`m3e-press relative flex flex-1 items-center justify-center gap-1.5 border-outline px-4 t-label-large text-on-surface-variant ${
            i > 0 ? 'border-l' : ''
          } ${i === selected ? 'bg-secondary-container/70 text-on-surface' : 'hover:bg-on-surface/6'}`}
        >
          {i === selected && <Icon name="check" className="text-[18px] text-on-secondary-container" />}
          {label}
        </button>
      ))}
    </div>
